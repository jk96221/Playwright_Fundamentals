...
Locator: locator('.selected-usernames')
Expected substring: "Meera.Rao"
Timeout: 5000ms
Error: element(s) not found

Call log:
  - Expect "toContainText" with timeout 5000ms
  - waiting for locator('.selected-usernames')
        ...
        - generic [ref=e231]:
            ...
            - rowgroup [ref=e252]:
              ...
              - row "Select Meera.Rao Meera.Rao MR Meera Rao EMP-1006 Automation Architect Mumbai Cloud Ledger Active" [ref=e328]:
                - cell "Select Meera.Rao" [ref=e329]:
                  - checkbox "Select Meera.Rao" [checked] [active] [ref=e330] [cursor=pointer]
          ...
          - generic [ref=e403]:
            - generic [ref=e404]:
              ...
              - generic [ref=e406]: 1 selected
            - generic [ref=e407]: Meera.Rao
...
```ts
  ...
  5  | 
  6  |   // Assuming there's a search input, but from the page description, it might be filter by team etc.
  7  |   // But user said "search for Meera.Rao", perhaps there's a search box.
  8  |   // If not, we can locate the row directly.
  9  | 
  10 |   // Locate the row containing Meera.Rao
  11 |   const row = page.locator('tr').filter({ hasText: 'Meera.Rao' });
  12 | 
  13 |   // Click the checkbox in that row
  14 |   await row.locator('input[type="checkbox"]').click();
  15 | 
  16 |   // Verify that Meera.Rao is selected
  17 |   // Assuming selected usernames are shown in a div with class 'selected-usernames' or similar
  18 |   const selected = page.locator('.selected-usernames');
> 19 |   await expect(selected).toContainText('Meera.Rao');
     |                          ^ Error: expect(locator).toContainText(expected) failed
  20 | });
  21 | 
```