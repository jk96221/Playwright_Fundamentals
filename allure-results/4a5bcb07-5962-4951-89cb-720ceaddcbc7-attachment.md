...
- Name: 12_SHADOW_DOM\Project#9_Shadow DOM, Calendars.spec.ts >> SelectorsHub Form Automation >> Enter name, pizza name and password
- Location: tests\12_SHADOW_DOM\Project#9_Shadow DOM, Calendars.spec.ts:17:9
...
Call log:
  - waiting for locator('//input[@placeholder=\'First Enter name\']')
    - locator resolved to <input value="" disabled name="test" class="nameFld" placeholder="First Enter name"/>
    - fill("Narasimha")
                ...
                - generic [ref=f2e8]:
                  ...
                  - generic [ref=f2e16]:
                    - link "Sign in to Google" [ref=f2e17] [cursor=pointer]:
                      - /url: https://accounts.google.com/Login?continue=https%3A%2F%2Fdocs.google.com%2Fforms%2Fd%2Fe%2F1FAIpQLSeguyGBfogdUO6IRfxJapNxTJQTaAEWYfcd5984L0T_q-0Syg%2Fviewform%3Ffbzx%3D-4972614576516460423
...
```ts
  ...
  4  | 
  5  | test.describe('SelectorsHub Form Automation', () => {
  6  | 
  7  |     test.beforeEach(async ({ page }) => {
  8  | 
  9  |         // Increase page timeout
  ...
  15 |     });
  16 | 
  17 |     test('Enter name, pizza name and password', async ({ page }) => {
  18 | 
  19 |         // Name
  20 |         const nameInput = page.locator(
  21 |             "//input[@placeholder='First Enter name']"
  22 |         );
  23 | 
  24 |         await nameInput.waitFor({ state: 'visible' });
> 25 |         await nameInput.fill('Narasimha');
     |                         ^ Error: locator.fill: Test timeout of 60000ms exceeded.
  26 | 
  27 |         // Pizza Name
  28 |         const pizzaInput = page.locator(
  29 |             "//input[contains(@placeholder,'pizza')]"
  30 |         );
  31 | 
  32 |         await pizzaInput.waitFor({ state: 'visible' });
  33 |         await pizzaInput.fill('Cheese Pizza');
  34 | 
  35 |         // Password
  ...
  38 |         );
  39 | 
  40 |         await passwordInput.waitFor({ state: 'visible' });
  41 |         await passwordInput.fill('Test@123');
  42 | 
  43 |         // Assertion
  44 |         await expect(nameInput).toHaveValue('Narasimha');
  45 | 
