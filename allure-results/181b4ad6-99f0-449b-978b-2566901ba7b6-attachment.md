...
```ts
  ...
  6  | 
  7  |     const frame1:FrameLocator = await page.frameLocator('#pact1');
  8  |     const frame2:FrameLocator = await frame1.frameLocator('#pact2');
  9  |     const frame3:FrameLocator = await frame1.frameLocator('pact3');
  10 | 
> 11 |     await frame1.locator('#inp_val').fill('Poornima');
     |                                      ^ Error: locator.fill: Error: strict mode violation: locator('#pact1') resolved to 2 elements:
  12 |     await frame2.locator('#jex').fill('Anusha');
  13 |     await frame3.locator('#glaf').fill('Marriage');
  14 | 
  15 |     const HeaderText = await page.locator('h3').innerText();
