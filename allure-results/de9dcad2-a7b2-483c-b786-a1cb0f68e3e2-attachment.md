...
Call log:
  - waiting for locator('input[placeholder="Search by name or email"]')
...
```ts
  ...
  7  | 
  8  |   // Wait for page to load completely
  9  |   await page.waitForLoadState('networkidle');
  10 | 
  11 |   // Search for Meera.Rao
> 12 |   await page.locator('input[placeholder="Search by name or email"]').fill('Meera.Rao');
     |                                                                      ^ Error: locator.fill: Test timeout of 30000ms exceeded.
  13 | 
  14 |     // Wait for filtered result
  15 |   await page.waitForTimeout(2000);
  16 | 
  17 |   // Locate checkbox for Meera.Rao
  18 |   const checkbox = page.locator(
  19 |     'tr:has-text("Meera.Rao") input[type="checkbox"]'
  20 |   );
  21 | 
  22 |   // Wait for checkbox to be visible
  23 |   await checkbox.waitFor({ state: 'visible' });
  24 | 
  25 |   // Select the checkbox
  26 |   await checkbox.check();
  27 | 
  28 |   // Verify checkbox is selected
  29 |   await expect(checkbox).toBeChecked();
  30 | 
  31 |   // Verify Meera.Rao appears in selected users section
  32 |   const selectedUser = page.locator('text=Meera.Rao').nth(1);
  33 | 
  34 |   // Wait for selected user text to appear
  35 |   await selectedUser.waitFor({ state: 'visible' });
  36 | 
  37 |   await expect(selectedUser).toBeVisible();
  38 | 
  39 |   console.log('Meera.Rao is selected successfully');
  40 | 
  41 |   // Runtime wait (Pause for 5 seconds)
