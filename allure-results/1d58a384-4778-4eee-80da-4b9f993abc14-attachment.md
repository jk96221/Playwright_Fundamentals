...
- Name: 09_Frames\09_1_IFRAME.spec.ts >> Handle Iframe
- Location: tests\09_Frames\09_1_IFRAME.spec.ts:3:5
...
Call log:
  - waiting for getByText('Submit registration', { exact: true })
                ...
                - generic [ref=f1e7]:
                  ...
                  - generic [ref=f1e18]:
                    ...
                    - generic [ref=f1e22]:
                      ...
                      - spinbutton "Year" [ref=f1e24]: "2022"
                  - generic [ref=f1e25]:
                    - generic [ref=f1e26]: Notes
                    - textbox "Notes" [active] [ref=f1e27]:
                      - /placeholder: Any notes about the registration
                      - text: Amezing Car for family and long drive
                  - generic [ref=f1e28]:
...
```ts
  ...
  2  | 
  3  | test('Handle Iframe',async({page})=>{
  4  |     
  ...
  9  | 
  10 |     
  11 |     const VechileFrame=page.frameLocator('#frame-one');
  12 | 
  13 |     await VechileFrame.locator('#RESULT_TextField-1').fill('Huandai-I10');
  14 |     await VechileFrame.locator('#RESULT_TextField-2').fill('Narasimharao');
  15 |     await VechileFrame.locator('#RESULT_TextField-3').fill('AP32ZA7832');
  16 |     await VechileFrame.locator('#RESULT_RadioButton-1').selectOption('SUV');
  17 |     await VechileFrame.locator('#RESULT_TextField-4').fill('2022');
  18 |     await VechileFrame.locator('#RESULT_TextArea-1').fill('Amezing Car for family and long drive');
  19 | 
> 20 |     await page.getByText('Submit registration',{exact:true}).click();
     |                                                              ^ Error: locator.click: Test timeout of 30000ms exceeded.
  21 | 
  22 |     let output = await VechileFrame.locator('#vehicle-output').innerText();
  23 | 
  24 |     console.log("Vechile Information:",output);
  25 |     
